...
function ARView({ mode, calibrado, pontoReferencia, pontos, onCreatePoint }) {
  ...
  const carregarPontosSalvos = () => {
    ...
    const pontosDoEvento = pontos.filter(ponto => 
      ponto.qrReferencia === pontoReferencia.qrCode
    )

    console.log(`Carregando ${pontosDoEvento.length} pontos salvos para modo ${mode}...`)
  ...
  const limparObjetosAR = () => {
    ...
    sceneRef.current.traverse((child) => {
      if (child.isMesh && (child.geometry?.type === 'BoxGeometry' || child.userData?.carregado)) {
